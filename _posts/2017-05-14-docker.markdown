...
About drawbacks, I've found two: The first one is the only possible debugging is remote debugging.
 It is rather comfortable for java projects using Idea, but probably it may lead to problems
 with other stack of instruments. The second
 problem is launching ui applications. Docker wasn't created for it, so if your development process
