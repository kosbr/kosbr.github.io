...
Good question is what if elasticsearch has been destroyed, but I need to see logs? There are at least two
approaches on this problem:

* It is not problem, if volumes are used for logs, all logs can be found directly in the nodes.
...
Bad situation is when additionally the application have broken and it is needed to see logs.
...
service is. There is no difference from outside, because swarm redirects requests from every node to necessary
one. It is better described in the official docker swarm documentation.

In real life it is impossible to completely rely on swarm. Some nodes are too weak for hosting some services,
...
I have 9 independent nodes (VDS) in the cluster, two of them are manager. According to the description above, 9 services should be deployed
at this cluster:
...
This is one more service, that is used for storing data. There is also special node for this service.

5. Storage service
6. Gate service

These services are designed to be launched in parallel with other same instances. The main difference with the
cassandraN services is that every instance of the storage or gate service doesn't know information
 about others. For example, one storage service can't know the number of another instances of this service.
 These services are stateless, so I don't care about hosting nodes, but docker swarm does.
...
Portainer is a just pretty UI for managing with cluster.

That's all about description of the services. As you may see from the picture, I have 9 nodes and 4 of them
are specially prepared for corresponding services. The rest services are distributed over the rest nodes. To
make it more clear, I use different colors for such "free" nodes and containers.

Docker swarm knows the parameters of every node (memory, CPU) and the demands and limits of every service. It
helps to achieve better balance and to avoid fails.

All services are connected by overlay docker network. It makes them visible for others. Those services,
...
Here is all my nodes from the command line of the manager node:

![Nodes](/images/articles/cluster/nodes-list.png)

And services with actual state:

![Services](/images/articles/cluster/services-list.png)

The portainer can prepare a good visualisation for a cluster:
...
If the cluster is well balanced (bad balanced means, for example, the same services are placed on the one
node) and then something happens... For example, some service or even node can be destroyed. No matter,
anyway docker swarm immediately got state that is not equal to wished one. Then some actions should be done.
If a leader manager was lost, another manager takes this role. Also docker will try to create new instances of
the services, probably on other nodes to make state to be as wished one. That's why, ideally, we should always
have the additional resources in the cluster. In my case, some services can't be recreated on other nodes -
it is cassandra services and elastic. However, that is why I have several cassandra services and it was
...
1. Use encryption in overlay docker network (built in docker, just switch on)
2. Close all ports unused from outside by firewall on the every node. All requests to open
...
It means, that these applications can be created with any technology. And that is the advantage of the stack
and architecture - we can use the best instrument for the
problem of microservice.
