...
The first one is responsible for data storage (actually it is a bridge to distributed database).
...
anyway docker swarm immediately got state that is not equal to the wished one.
...
2. Close all unused ports from outside by firewall on the every node. All requests to open
...
and architecture - we can use the best instrument for the problem of the microservice.
